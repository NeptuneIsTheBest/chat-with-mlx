...
from .chat_types import PreparedPrompt, normalize_turn_media_sequence
...
class PromptCachePlanner:
    def prepare_chat_generation(
        ...
        if self._is_multimodal_model(model):
            return self._prepare_multimodal_generation(model, prepared_prompt, cache_session_id, entry)
        if self._is_text_model(model):
            return self._prepare_text_generation(model, prepared_prompt, cache_session_id, entry)
        return None
    ...
    def _prepare_text_generation(
        ...
        prompt_token_ids = list(prepared_prompt.prompt_token_ids)
        ...
        return PromptCachePlan(
            ...
            backend="text",
            prompt=prepared_prompt.formatted_prompt,
    ...
    def _prepare_multimodal_generation(
        ...
        prepared_inputs = prepared_prompt.prepared_inputs or model.prepare_prompt_inputs(
            prepared_prompt.formatted_prompt,
            images=prepared_prompt.flat_images,
            audios=prepared_prompt.flat_audios,
        )
        prompt_token_ids = list(prepared_prompt.prompt_token_ids)
        media_sequence = normalize_turn_media_sequence(prepared_prompt.turn_media_sequence)
        prompt_cache = model.make_prompt_cache()
        cached_prefix_len = 0
        cached_token_prefix_len = 0
